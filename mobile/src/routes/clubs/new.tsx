...
function RouteComponent() {
  ...
  const [clubInviteCode, setClubInviteCode] = useState<string>("");
  ...
  const joinClubMutation = useMutation({
    mutationFn: (code: string) => api.clubs.joinClub(code),

    onSuccess: async (newClub) => {
      console.log("✅ Club rejoins:", newClub);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    onError: (error: any) => {
      console.error("❌ Erreur join club:", error);
      alert(`Erreur: ${error.message || "Impossible de rejoidnre le club"}`);
    },
  });

  const handleSubmitCreateClub = (e: React.FormEvent) => {
    e.preventDefault();
    if (!clubName.trim()) {
  ...
  const handleSubmitJoinClub = (e: React.FormEvent) => {
    e.preventDefault();
    if (!clubInviteCode) {
      alert("Le code d'invitation est requis");
    }

    joinClubMutation.mutate(clubInviteCode)
  }
      ...
      <div className="w-full max-w-sm">
        <Tabs defaultValue="create">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="create">Créer ton club</TabsTrigger>
            <TabsTrigger value="join">Rejoins ton club</TabsTrigger>
          </TabsList>
          <TabsContent value="create">
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleSubmitCreateClub} className="space-y-4">
                  <div>
                    <label
                      htmlFor="clubName"
          ...
          <TabsContent value="join">
            <Card>
              <CardHeader>
                <CardTitle>Rejoins un club ⚾</CardTitle>
                <CardDescription>
                  Commencez par rejoindre votre premier club
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmitCreateClub} className="space-y-4">
